...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const closeImg = document.querySelector('dialog span');
  ...
  for (let i = 0; i < picArray.length; i++) {
    ...
    title.appendChild(document.createTextNode(picture.title));
    const fig = document.createElement(`figure`);
    const img = document.createElement(`img`);
    img.src = picture.image.medium;
    img.alt = picture.title;
    const figcap = document.createElement(`figcaption`);
    figcap.appendChild(document.createTextNode(picture.caption));
    const t = document.createTextNode(picture.description);
